 ...
 */
function getTopText () {
    $message = "You received a new message sent via the contact form on brackenburycapital.com.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on brackenburycapital.com.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "brackenburycapital.com"]) . ">",
        "Date: ".date("r (T)"),
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "brackenburycapital.com") {
...
function processMail() {
    ...
    if (empty($response)) {
        ...
        $contactFormEmails = array_map("toLower", json_decode('["info@brackenburycapital.com"]'));
        if (
